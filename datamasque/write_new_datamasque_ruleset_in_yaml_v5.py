...
def filter_yaml(yaml_content, to_mask_tables_and_fields, all_tables_and_columns_dict):
    ...
    for entry in yaml_content:
        ...
        # If key is REPLACE_ME, skip entry
        if entry.get('key'):
                if str(entry.get('key')) == '"REPLACE_ME"':
                    continue
            ...
            for rule in entry.get('rules', []):
                ...
                for column in table_columns:
                    ...
                    if rule['column'] == column:
                        ...
                        patient_id_check_list = [column.lower().strip('"').strip("'") for column in all_tables_and_columns_dict[schema_name][table_name]]

                        # check if patientid or patient_id already exist in hash_columns for the specific rule being processed
                        if ('patientid' == rule_column_lower and 'patientid' not in hash_columns) or 'patientid' in patient_id_check_list:
...
def main():
    ...
    with open('filtered_v5.yaml', 'w') as f:
        yaml.dump(filtered_content, f, indent=2, sort_keys=False, default_flow_style=False)
