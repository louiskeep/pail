...
def filter_yaml(yaml_content, to_mask_tables_and_fields, all_tables_and_columns_dict):
    ...
    for entry in yaml_content:
        ...
        table_name = entry.get('table').split('.')[1]
        
        if entry.get('type') == 'mask_table' and table_name in to_mask_tables_and_fields:
            if table_name == 'Letter_Attachment':
                print("Got it")
            ...
            # Get list of all fields in DB table
            #table_columns = [record.lower() for record in all_tables_and_columns_dict.get(schema_name, {}).get(table_name, [])]

            for rule in entry.get('rules', []):

                rule_column_lower = rule['column'].strip('"').lower()
                if rule['column'] in to_mask_tables_and_fields[table_name]:
                    ...
                    if 'patientid' == rule_column_lower and 'patientid' not in hash_columns:
...
def main():
    ...
    with open('filtered.yaml', 'w') as f:
